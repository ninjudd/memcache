...
or record_cache[http://github.com/ninjudd/record_cache]. +Memcache::Server+ implements
...
The Memcache::NativeServer backend provides native bindings to libmecached. This is
significantly faster than using Memcache::Server as demonstrated by runnning
...
[+hash+] The libmemcached hashing method. One of +:default+, +:md5+, +:crc+, +:fnv1_64+,
         +:fnv1a_64+, +:fnv1_32+, +:fnv1a_32+, +:jenkins+, +:hsieh+, +:murmur+. NOTE: Even
         thought there is a libmemcached method named +:default+ (which is actually Jenkins's
         one-at-a-time hash), the default hashing method if you don't specify one is +:crc+.
